...
These are direct lifecycle commands you run yourself in the agent host when the workflow requires them — slash commands for some agents, skills for Codex, and never a separate `feature-submit` command. The `aigon agent-status` command writes state to the **main repo** (not the worktree), so you won't see state files locally. Just run the command and trust the output.
