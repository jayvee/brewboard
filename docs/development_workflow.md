...
5. Run `aigon feature-done <ID>` — the gate checks the PR and then either:
   - proceeds with the normal local close when the PR is open, non-draft, and mergeable
   - or syncs the default branch from origin and completes cleanup when the PR is already merged
