...
If you used `feature-start <name>` or `feature-start <name> <agent>`:
```bash
...
### Fleet mode
If you used `feature-start <name> cc gg cx cu`:
```bash
aigon feature-close <name> <winning-agent>
```

Example: `aigon feature-close <name> cc` to merge Claude's implementation

### Fleet mode with adoption
Merge the winner and print diffs from losing agents for selective adoption:
```bash
aigon feature-close <name> <winning-agent> --adopt <agents...|all>
```

Examples:
- `aigon feature-close <name> cc --adopt gg cu` — adopt from specific agents
- `aigon feature-close <name> cc --adopt all` — adopt from all losing agents
...
- **Remote branch gate**: If `honourRemoteBranchGate` is enabled in `.aigon/config.json`, close checks the feature's GitHub PR before proceeding. Push the branch first with `/aigon-feature-push <name>`. If the PR is already merged, Aigon syncs the default branch from origin and completes cleanup instead of performing a second local merge.
