...
Parse the `<slug>` out of the branch name (strip the `feature-` prefix and the trailing `-<agent>` suffix where agent is one of `cc, cx, cu, op, km, am, ag, cp`). Then resolve the slug to a feature ID by matching against `aigon feature-list --active`:
