...
- **Remote branch gate**: If `honourRemoteBranchGate` is enabled in `.aigon/config.json`, close checks the feature's GitHub PR before proceeding. Push the branch first with `/aigon:feature-push {{args}}`. If the PR is already merged, Aigon syncs the default branch from origin and completes cleanup instead of performing a second local merge.
